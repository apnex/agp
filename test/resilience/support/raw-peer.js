...
export async function connectRawPeer({
  network,
  nodeId,
  targetNodeId,
  port,
  ...
  const transport = network.transport(nodeId, {
    listeners: [],
    targets: [[transportRef, `chaos://network/port-${port}`]],
  });
  const target = transport.resolveTarget(transportRef);
  if (target === undefined) throw new Error("raw chaos target did not resolve");
  ...
  const pump = (async () => {
    while (!terminal) {
      ...
      if (parsed.message.type === "route.update") {
        await sendProtocol({
          agp: 1,
          plane: "control",
          type: "route.ack",
  ...
  return Object.freeze({
    ...
    waitForTerminal() {
      return take(
        ...
        `${nodeId} terminal event`,
      );
    },
    waitForMessage(select) {
      return take(
        inbox,
        (item) => item.kind === "message" && select(item.message),
        `${nodeId} selected message`,
      ).then((item) => item.message);
    },
    async close() {
      if (!terminal) {
