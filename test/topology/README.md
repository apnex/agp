...
Every node vertex is created through public `createNode()` and every topology
uses a real loopback WebSocket. Raw peers appear only where the primary
contract is an exact authoritative snapshot or physical-session close.

| File | Sole primary axis | Explicit non-goal |
|---|---|---|
| `star-convergence.test.js` | Multi-endpoint spoke reachability through one center | Alternative-path policy |
