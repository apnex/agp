...
| `record-integrity.test.js` | What one record asserts about another holds: board state agrees with its own ledger, a cited board item exists, an amended intent names its decision, and a ratified decision is resolved against the vision | cross-record integrity | ledger status against placement, milestone membership and closed count, citation resolution, amendment references, and the absorption register |
