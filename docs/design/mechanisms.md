...
|---|---|---|---|---|---|---|
| M01 | Uniform node | One `createNode()` implementation composes listen, dial, endpoint, routing, and transit capabilities; topology position is configuration, not a role. | AGP-native | - | [`ARCHITECTURE.md` section 2](../ARCHITECTURE.md#2-mandate), [`sdk.md` section 3](sdk.md#4-public-node-api) | AX5 |
...
| M03 | Symmetric peer session | Once established, either acquisition kind may send every legal route/control/data message. Internal `dial|accept` controls reconnect ownership only; public `outbound|inbound` is its fixed read-only projection. | Adapted | BGP peer sessions in [RFC 4271](https://www.rfc-editor.org/rfc/rfc4271) | [`protocol.md` sections 1,8](protocol.md#1-mandate), [`fsm.md` section 1](fsm.md#1-model) | AX3 |
| M04 | Connection FSM | Retain `Idle`, `Connect`, `Active`, `OpenSent`, `OpenConfirm`, and `Established`; require OPEN and received KEEPALIVE before establishment. Reconnect supervision is outside the per-connection FSM. | Adapted | RFC 4271 sections 8-10 | [`fsm.md`](fsm.md) | AX3 |
...
| M07 | Hold and keepalive timers | Zero disables both; otherwise keepalive is one third of negotiated hold. Any valid inbound AGP envelope proves peer liveness; any successful outbound envelope suppresses unnecessary keepalive. | Adapted | RFC 4271 sections 4.4, 6.5, 8, 10 | [`fsm.md` section 5](fsm.md#5-timers) | AX3 |
| M08 | Stalled-writer protection | Every write and close is finitely timed; queues are bounded; inability to send mandatory control traffic tears down the session rather than retaining stale routing state. | Aligned in safety outcome; different timer granularity | [RFC 9687](https://www.rfc-editor.org/rfc/rfc9687) Send Hold Timer | [`fsm.md` sections 5,6](fsm.md#5-timers), [`routing.md` section 7.4](routing.md#74-withdrawal-ordering-barrier) | AX3, AX5 |
...
| M12 | Ordered path vector | Each route carries an ordered unique node path from origin through advertiser. Reject a received path containing the local node and suppress export to a peer already in the path. | Aligned conceptually | RFC 4271 `AS_PATH` / AS-loop exclusion | [`protocol.md` sections 4-5](protocol.md#4-authoritative-route-snapshots), [`routing.md` sections 5,7](routing.md#5-route-import) | AX2, AX4 |
| M13 | Selected-route export | Only a selected, resolvable route is considered for a peer's Adj-RIB-Out; learned routes additionally require local transit permission. | Aligned conceptually, with AGP policy | RFC 4271 sections 3.2, 9.1.3 | [`routing.md` section 7](routing.md#7-selected-route-export) | AX4 |
...
| M19 | Acknowledged source-export barrier | Before peer data egress, that peer must have ACKed the same selected source identity in its Adj-RIB-Out. Withdrawal closes the export epoch before later data admission. | AGP-native | Related to control/data-plane consistency, but no BGP packet-plane equivalent is claimed | [`routing.md` sections 7.3,8](routing.md#73-source-export-barrier), [`protocol.md` section 5.1](protocol.md#51-routedata-write-ordering) | AX4, AX5 |
| M20 | RIB-gated JSON data plane | Local send and transit resolve the destination through the same selected RIB/FIB. Missing or unusable route emits zero onward data packets. | AGP-native | - | [`routing.md` section 9](routing.md#9-uniform-data-forwarding), [`ARCHITECTURE.md` section 8](../ARCHITECTURE.md#8-canonical-processing-paths) | AX5 |
...
| M22 | Correlated reverse delivery dispositions | One message reports the fate of a message, delivery or failure, batched per session. Prior forwarders use bounded hop-local label bindings and never consult the RIB, and a binding is released by the report that returns for it. Nonfatal outcomes do not reset a valid session. | AGP-native | BGP NOTIFICATION is intentionally not reused because it is fatal and control-plane scoped | [`protocol.md` section 7](protocol.md#7-correlated-reverse-dispositions), [`routing.md` section 10](routing.md#10-reverse-delivery-dispositions) | AX2, AX5 |
| M23 | Fatal notification | Schema/identity/revision ambiguity sends a typed notification when safe, purges session-owned routes, and closes the session channel. | Aligned in lifecycle semantics; AGP codes differ | RFC 4271 sections 4.5, 6 | [`protocol.md` section 9](protocol.md#9-fatal-versus-recoverable-failures), [`fsm.md` sections 6-7](fsm.md#6-transition-tables) | AX2, AX3 |
...
| M25 | Symmetric learned-route propagation | Any peer may receive a selected learned route when transit and loop rules permit. AGP has no iBGP/eBGP split-horizon or route-reflector role. | Departure | RFC 4271 section 9.2.1 internal-peer redistribution rule | [`protocol.md` sections 1,5](protocol.md#1-mandate), [`routing.md` section 7](routing.md#7-selected-route-export) | AX4, AX7 |
| M26 | Ephemeral routing state | Restart starts with empty sessions/RIB/FIB/export/label binding state and reconverges from endpoint intent and fresh snapshots. | Departure for v1 | [RFC 4724](https://www.rfc-editor.org/rfc/rfc4724) graceful retention is not adopted | [`DECISIONS.md` D9](../DECISIONS.md#d9---volatile-derived-runtime-state), [`sdk.md` section 9](sdk.md#6-restart-and-state-lifetime) | AX7 |
| M27 | Operational visibility | Every node exposes connection FSM, timers, Adj-RIB-In, candidates, selected RIB, FIB, Adj-RIB-Out, resources, and counters through canonical SDK state, read-only HTTP, and `agpctl`. | Adapted operator model | BGP table/session operational practice; no CLI syntax compatibility is claimed | [`sdk.md` sections 5-8](operations.md#3-canonical-operational-state) | AX6 |
...
| M35 | Pre-identity controller boundary | A transferred channel is observable before OPEN admission only as a node-wide-local pre-identity controller with no remote identity or routing authority. Admission atomically replaces it with a pair-scoped session; teardown emits exactly one mutually exclusive pre-identity or admitted-session event. | AGP-native | Related to BGP's pre-OpenConfirm identity boundary, but the sovereign operational projection is AGP-specific | [`protocol.md` section 3](protocol.md#3-open), [`fsm.md` sections 3,7](fsm.md#3-controller-state-and-invariants), [`contracts.md` section 6.4](contracts.md#64-required-state-semantics), [`sdk.md` section 5.1](operations.md#31-uniform-entity-semantics) | AX3, AX6 |
| M36 | Pre-shared-key channel protection | A binding may protect channels with a pre-shared secret selected by identity. `network` keying shares one secret and proves group membership; `node` keying gives each node its own and lets a listener prove which peer connected. Evidence reports exactly what the handshake established and never the secret. Specified for star and line topologies. | Adapted | TLS pre-shared keys ([RFC 4279](https://www.rfc-editor.org/rfc/rfc4279)), TLS 1.3 ([RFC 8446](https://www.rfc-editor.org/rfc/rfc8446)) | [`transport-contract.md` section 16](transport-contract.md#16-channel-security), [`binding-websocket.md` section 10](binding-websocket.md#10-secure-profile), [`DECISIONS.md` D18](../DECISIONS.md#d18---pre-shared-key-transport-security) | AX1-T, AX1-B |
| M37 | Per-hop credit flow control | A receiver grants byte and packet credit per adjacency and direction; a sender may not admit beyond it. Credit rides every envelope while traffic flows and `KEEPALIVE` while idle, and never exceeds the receiver's supplied channel limits. Exceeding a grant is fatal, never a drop. | Adapted | TCP receive window ([RFC 9293](https://www.rfc-editor.org/rfc/rfc9293)); PCIe split header and data credits | [`DECISIONS.md` D19](../DECISIONS.md#d19---per-hop-credit-flow-control) | AX2, AX3, AX5 |
...
| F09 | Cumulative delivery acknowledgement | Batched acknowledgement is built as the `D23` disposition: a label binding is released by the report that returns for it, whether that report is a delivery or a failure. What remains deferred is the cumulative part, and only that. | TCP cumulative and delayed acknowledgement ([RFC 9293](https://www.rfc-editor.org/rfc/rfc9293)); DTN custody signals ([RFC 5050](https://www.rfc-editor.org/rfc/rfc5050)) | Requires a reason to believe reports arrive in order. `D23` rejected cumulative acknowledgement rather than deferring it on cost, because dispositions genuinely complete out of order: a failure two hops away returns sooner than a success four hops away. Re-entry needs that premise to change, not a deployment. The upstream-timing question this row used to hold was ruled in `D23`: a hop reports once its own downstream has reported to it. |
...
| F11 | Store-and-forward relay | A node forwards or refuses and never accepts custody on behalf of a destination. | SMTP relay; DTN bundle custody | Requires a deployment that needs it. The `MX7` precondition this row carried is met: a relay is the node most likely to sustain a high rate, and `D23` removed the ceiling that would have met it first. It is an application addressed as a destination, not a routing-plane capability. |
| F12 | Per-flow labels | A label identifies one message, so the label table is proportional to traffic. | MPLS label-switched paths, where a label identifies a forwarding class | Requires flow-aware forwarding: per-flow credit, per-flow path selection, or per-flow fairness in a transit queue. Those need a transit node to know a flow exists and cannot be reached by any arrangement of endpoint state. Throughput is not a trigger, and neither is flow semantics: sequencing, gap detection and reliable delivery are reachable by a layer above, on the demarcation in [`message-labels.md`](message-labels.md) section 7. The design is written out in full there in section 5 and contrasted in section 6. |
| F13 | Replication to every advertiser of an endpoint | A destination resolves to one next hop, and `forwardingEntry` returns a single one by type. | Point-to-multipoint label-switched paths; network-layer multicast | Requires a use for one message reaching every node advertising a name. The addressing generalises as a fourth destination mode and is recorded in [`destination-selection.md`](destination-selection.md) section 5; the forwarding does not, needing a fan-out point, loop handling across branches, deduplication where branches reconverge, per-branch credit and queue accounting, and one disposition per branch. |
| F14 | Strict RPF replication | Source validation is feasible-path RPF under `D7` and `M18`, deliberately weaker than strict so that legitimate asymmetric best paths are permitted. | [RFC 3704 section 2.3](https://www.rfc-editor.org/rfc/rfc3704#section-2.3) strict RPF; multicast reverse-path trees | Requires fan-out that wants delivery without accounting, because a copy dropped by strict validation is discarded silently and yields no per-destination outcome, no intermediate count, and no completeness at the origin. It also requires accepting that a node holds two source-validation rules at once, disagreeing precisely in the asymmetric cases `D7` exists to permit. Contrasted against the carried list in [`destination-selection.md`](destination-selection.md) section 5.4. |
...
`F09` is built, as the `D23` disposition in [`message-labels.md`](message-labels.md), which reports success as well as failure along the same reverse path.\
Only its cumulative form remains deferred, and that one was refused on correctness rather than held on cost.\
What follows is why it sat in this layer, and it is why the built mechanism sits there too.\
`F09` belongs to the AGP session layer, beside credit.\
A carrier cannot release a label binding because it does not know one exists, and an application cannot release one because it does not own it.\
...
Label bindings, acknowledgement state and pending calls are each a bounded store keyed by correlation and cleared by expiry.\
