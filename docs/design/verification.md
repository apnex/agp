...
| AX1-B | `packages/transport-node-ws/test/contract/trusted-development-profile.test.js` | The certified adapter accepts only trusted-development `ws:`, emits exact network/none/unauthenticated evidence, and rejects WSS/TLS/auth configuration before resolver construction |
