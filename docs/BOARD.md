...
| ID | Move | Note |
|---|---|---|
| `B23` | Build the disposition design in `message-labels.md`: completion reporting per session, ranges with outcome codes, eviction over refusal, and the rename to label bindings | The design is settled and written down, including the per-flow alternative and why it is not being built. What remains is the wire shape, the debounce and count defaults, the SDK surface, and the fourth correlation store at the origin, which must be released by success by construction |
| `B26` | Let a ratified decision be recorded before it is built | `B17` made the trace graph derive its decision set from the record, which is right, and made every decision require an existing owning test, which is too strong. The corpus recognises a ratified but unbuilt decision as a legitimate state; `B12` is triggered by exactly that. Today such a decision cannot be written down without failing `AX0`, so the design in `message-labels.md` is a design of record rather than a `D` entry until it is built |
...
| Question | Blocks |
|---|---|
| Whether the disposition vocabulary carries progress as well as outcome | `B23`, which builds that vocabulary, though the consumer is `F13`. An origin that let a downstream hop enumerate the recipients cannot know how many to expect, so it counts what arrives and never learns it is complete. The available answer is a fan-out report from the dividing hop, which describes the journey rather than the destination. Every code in the set today names an outcome, and adding this as one more value would put a second kind of thing into a vocabulary holding one kind; deciding now costs nothing and deciding later costs the shape |
