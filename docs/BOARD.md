...
| `B23` | Decide whether the sustained send ceiling is the intended one | `I2` | `P3` | [`MX7`](VERIFICATION.md#46-open-findings-from-sweeps), [`F09`](design/mechanisms.md) | open |
...
| ID | Move | Note |
|---|---|---|
| `B23` | Decide whether a sustained ceiling of roughly 136 messages a second is the intended one | `MX7`, reframed by discussion rather than by measurement. A breadcrumb is released by failure or expiry and never by success, so a flow that never fails still fills the store, and the window bounds reverse-error reach rather than correctness. The decision is therefore not what number to raise it to. It is whether a quality knob should keep acting as a throughput cap, and `F09` records the acknowledgement that would remove the question |
