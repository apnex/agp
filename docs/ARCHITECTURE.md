...
Per-section maturity, because one status over eleven sections either overstates the weakest or is blocked by it.
...
| Ruling authority | [`DECISIONS.md`](DECISIONS.md) - `D1` through `D19` |
...
The upper layers are cited and not contained: the north star is held by [`VISION.md`](../VISION.md) and the rulings by [`DECISIONS.md`](DECISIONS.md), and this document takes on neither duty.
...
| Resource governance | What may be consumed, locally and by a peer | `core/capacity-ledger`, and `D19` credit as its peer-facing half |
| Time and liveness | Deadlines, keepalive, hold, retry | `core/session-fsm` timers |
| Observability | What the system reports about itself | `core/operations-store` |
...
|---|---|
| Deferred mechanisms, each with a re-entry condition | [`design/mechanisms.md`](design/mechanisms.md) `F01`-`F07` |
